...
use sql_middleware::middleware::{
    AsyncDatabaseExecutor, ConfigAndPool, DatabaseType, MiddlewarePool, MiddlewarePoolConnection,
    RowValues,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let users = vec![
        (
            ...
            true,
            "2024-01-15 10:30:00",
            r#"{"role": "admin", "preferences": {"theme": "dark"}}"#,
        ),
        (
            2,
            "Bob Smith",
            ...
            true,
            "2024-01-16 14:20:00",
            r#"{"role": "user", "preferences": {"theme": "light"}}"#,
        ),
        (
            3,
            "Carol Davis",
            ...
            false,
            "2024-01-17 09:15:00",
            r#"{"role": "moderator", "preferences": {"theme": "auto"}}"#,
        ),
    ];
    ...
    let posts = vec![
        (
            ...
            1,
            "Welcome to LibSQL!",
            "This is my first post using LibSQL with Rust.",
        ),
        (
            2,
            1,
            "Performance Comparison",
            "LibSQL shows great performance improvements over traditional SQLite.",
        ),
        (
            3,
            2,
            "Getting Started Guide",
            "Here's how to get started with the sql-middleware crate.",
        ),
    ];
    ...
    let result_set = libsql_conn
        .execute_select(join_query, &[RowValues::Bool(true)])
        .await?;
    ...
    let stats_result = libsql_conn
        .execute_select(stats_query, &[RowValues::Bool(true)])
        .await?;
    ...
    println!(
        "  • Total users: {}",
        stats_row.get("user_count").unwrap().as_int().unwrap()
    );
    println!(
        "  • Average age: {:.1}",
        stats_row.get("avg_age").unwrap().as_float().unwrap()
    );
    println!(
        "  • Total balance: ${:.2}",
        stats_row.get("total_balance").unwrap().as_float().unwrap()
    );
    println!(
        "  • Highest balance: ${:.2}",
        stats_row.get("max_balance").unwrap().as_float().unwrap()
    );
    ...
    let updated_rows = libsql_conn
        .execute_dml(update_sql, &[RowValues::Bool(true)])
        .await?;
    ...
    let updated_result = libsql_conn
        .execute_select(verification_query, &[RowValues::Bool(true)])
        .await?;
