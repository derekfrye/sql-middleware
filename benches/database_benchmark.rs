...
fn generate_insert_statements(num_rows: usize) -> String {
    ...
    for i in 0..num_rows {
        ...
        let timestamp_year = rng.gen_range(2020..=2025);
        let timestamp_month = rng.gen_range(1..=12);
        let timestamp_day = rng.gen_range(1..=28); // Avoiding edge cases with days per month
        let timestamp_hour = rng.gen_range(0..=23);
        let timestamp_minute = rng.gen_range(0..=59);
        let timestamp_second = rng.gen_range(0..=59);
        ...
        let blob_len = rng.gen_range(10..=20);
        let blob: Vec<u8> = (0..blob_len).map(|_| rng.gen_range(0..=255)).collect();
        let blob_hex = blob.iter()
            .map(|b| format!("{:02X}", b))
            .collect::<String>();
        ...
        let insert = format!(
            ...
            a, b, timestamp, d, if e { 1 } else { 0 }, blob_hex, json_value,
            additional_texts[0], additional_texts[1], additional_texts[2], 
            additional_texts[3], additional_texts[4], additional_texts[5],
            additional_texts[6], additional_texts[7], additional_texts[8]
        );
...
async fn setup_postgres_db(db_user: &str, db_pass: &str, db_name: &str) -> Result<(ConfigAndPool, common::postgres::PostgresContainer), Box<dyn std::error::Error>> {
...
fn benchmark_postgres(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("postgres", "500k_rows"), |b| {
        b.to_async(&rt).iter(|| async {
            ...
            let (config_and_pool, postgres_stuff) = setup_postgres_db(db_user, db_pass, db_name).await.unwrap();
