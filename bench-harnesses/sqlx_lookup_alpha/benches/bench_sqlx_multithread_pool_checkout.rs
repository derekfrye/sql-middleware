...
use sqlx::{
    ...
    ConnectOptions, Executor, Row, SqlStr, SqlitePool,
};
...
async fn sqlx_parallel_prepare(pool: &SqlitePool, concurrency: usize) -> Result<(), sqlx::Error> {
    ...
    for _ in 0..concurrency.max(1) {
        ...
        join_set.spawn(async move {
            let mut conn = pool.acquire().await?;
            let statement = conn.prepare(SqlStr::from_static(SQLITE_SELECT)).await?;
            drop(statement);
            Ok::<(), sqlx::Error>(())
        });
    }
...
fn sqlite_multithread_pool_checkout_sqlx(c: &mut Criterion) {
    let dataset = &*DATASET;
    let mut group = c.benchmark_group("sqlite_multithread_pool_checkout_sqlx_alpha");
